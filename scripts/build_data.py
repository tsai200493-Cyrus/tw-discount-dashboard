...
def load_watchlist():
    ...
    @since 把「估值位階」的比較起點釘死在某一天，算成 data.json 的 alt，
    只有網站「客製」模式用；標準那套永遠照近 5 年滾動算。
    起算日＝「這家公司現在這個樣子」的全部歷史，正當理由只有兩種：
      ① 業態變過（季營益率有站得住的新台階）→ 從台階那一季起算，區間變短
      ② 循環股（營益率大起大落、會掉回來）→ 拉長到涵蓋完整循環，區間變長
    ...
    ［更新 2026-09-15｜需求人］十檔全部設了 @since（需求人要求，日期交給主對話決定）。
      前提仍成立（縮短區間實測仍一律變便宜：台達電若設 2025-01，便宜度 +30），所以新設的
        8 檔起算日「全部早於」近 5 年的起點，沒有一檔是縮短；8 檔合計便宜度 −16（變貴、較保守）。
        判準（營益率台階站不站得住、循環股拉長）寫在 watchlist.txt 檔頭。
      已知代價：拉長到 2010 的 5 檔，Action 與瀏覽器歷史圖每次都要多抓約 3 倍的 PER 資料。
